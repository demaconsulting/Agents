# Introduction

<!-- TODO: Replace {ProjectName} with your project name. -->

This document defines the formal code review plan for {ProjectName}.

## Purpose

To define review-sets and the evidence required to demonstrate that each software item
has been reviewed according to the project's compliance requirements.

## Scope

This document covers all review-sets defined in `.reviewmark.yaml` for {ProjectName}.

## References

<!-- TODO: List references to related documents and external standards.
     - External links must be absolute URLs.
     - References to other project documents should be by document name only
       (e.g., "{ProjectName} User Guide"), not relative file links, as this
       document is compiled to PDF.
     If there are no references, leave this section as N/A. -->

N/A
